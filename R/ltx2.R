...
ltx2.coxreg <- function(x, caption = NULL, label = NULL, dr = NULL, 
    ...
    ltxCoef3(x, digits, dr, caption) # Print regression coefficients
### New 2020-11-26:    
...
ltx2.phreg <- function(x, caption = NULL, label = NULL, dr = NULL, 
    ...
    dr <- drop1(x, test = "Chisq") # NOTE: remove dr in arglist
...
ltxCoef3 <- function(x, digits, dr, caption, conf = 0.95, label = NULL){
    coef <- x$coef
...
##                      format = "f")
    ...
    ## Check for dr:
    lp <- TRUE
    if (is.null(dr)) stop("Missing required 'dr'.") 
    if (lp){
