...
```{r livingalone}
utd$b3h[is.na(utd$b3h)] <- 0
utd$livingalone <- factor(utd$b3h, labels = c("No", "Yes"))
```

## Diabetes
```{r diabetes}
utd$diabetes <- cut(utd$diabwho, c(-1, 0.5, 3.5, 4.5), 
                    labels = c("No", "Prediabetes", "Diabetes"))
...
```{r save}
##vip$diabetes <- as.character(vip$diabetes)
##vip$diabetes[is.na(vip$diabetes)] <- "unknown"
##vip$diabetes <- factor(vip$diabetes)
##
##vip$dependent <- as.character(vip$dependent)
##vip$dependent[is.na(vip$dependent)] <- "unknown"
##vip$dependent <- factor(vip$dependent)
##
vip$h1a <- vip$h1b <- vip$h1c <- vip$h1d <- vip$h1e <- vip$h1f <- vip$h1g <- NULL
...
Note the new variable *inc.90*: It is the *quartiles* of *income.90*.
...
vip$bmi <- vip$bmi - 25
vip$sbt <- vip$sbt - 120
br <- quantile(vip$income.90, na.rm = TRUE)
vip$inc.90 <- cut(vip$income.90, breaks = br, 
                  labels = paste0("Q", 1:4))
##vip$inc.90 <- relevel(vip$inc.90, ref = "Q1")
saveRDS(vip, file = "Rdata/vip.rds")
```

# Testing output format

V skippar det nu. (Tjafs med missing i 'green'.)
