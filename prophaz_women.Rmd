---
title: "Proportional hazards, women"
...
```{r gralder, fig.cap = "Stratification wrt alder.VIP, women. log-log scale."}
...
zz <- cox.zph(fit)
tbl(zz$table, caption = "Test of proportionality, women.", 
    rownames = TRUE)
```
...
fit.ph <- coxph(Surv(exit, event) ~ snususe + us.ar + late 
                    + strata(alder.VIP), data = vip)
ltx2(fit.basic, label = "tab:basic", caption = "Basic model, women.")
```
...
ltx2(fit.basic, label = "tab:basic2", caption = "Stratified by alder.VIP, women.")
```
...
fit.ph <- coxph(Surv(exit, event) ~ snususe + livingalone +
                ...
                , data = vip0[vip0$alder.VIP == "30", ])
ltx2(fit.basic, label = "tab:basic230", caption = "alder.VIP = 30, women.")
```
...
fit.ph <- coxph(Surv(exit, event) ~ snususe + livingalone +
                ...
                , data = vip0[vip0$alder.VIP == "40", ])
ltx2(fit.basic, label = "tab:basic240", caption = "alder.VIP = 40, women.")
```
...
fit.ph <- coxph(Surv(exit, event) ~ snususe  + livingalone +
                ...
                , data = vip0[vip0$alder.VIP == "50", ])
ltx2(fit.basic, label = "tab:basic250", caption = "alder.VIP = 50, women.")
```
...
fit.ph <- coxph(Surv(exit, event) ~ snususe + livingalone +
                ...
                , data = vip0[vip0$alder.VIP == "60", ])
ltx2(fit.basic, label = "tab:basic260", caption = "alder.VIP = 60, women.")
```
