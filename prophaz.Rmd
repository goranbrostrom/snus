...
Works well with a categorical (*factor*) covariate. We take *alder.VIP* and *sex* 
as examples. First *alder.VIP*, see Figure \@ref(fig:gralder).

```{r gralder, fig.cap = "Stratification wrt alder.VIP. log-log scale."}
fit <- coxreg(Surv(exit, event) ~ strata(alder.VIP) + sex, data = vip)
...
Next, *sex*, see Figure \@ref(fig:grsex).

```{r grsex, fig.cap = "Stratification wrt sex. log-log scale."}
fit <- coxreg(Surv(exit, event) ~ alder.VIP + strata(sex), data = vip)
plot(fit, col = 1:4, lty = c(1, 2, 4, 5),
     printLegend = "topleft", fn = "loglog")
```

Proportional hazards seem plausible, with a small reservation for the first year.
...
```{r zph, results = 'asis'}
fit <- coxph(Surv(exit, event) ~ alder.VIP + sex, data = vip)
zz <- cox.zph(fit)
...
fit.basic <- coxreg(Surv(exit, event) ~ snususe  + sex + us.ar + late 
                    + strata(alder.VIP), data = vip)
fit.ph <- coxph(Surv(exit, event) ~ snususe + sex + us.ar + late 
                    + strata(alder.VIP), data = vip)
ltx2(fit.basic, label = "tab:basic")
```
...
vip0 <- vip[!is.na(vip$education) &
               !is.na(vip$inc.90), ]
fit.basic <- coxreg(Surv(exit, event) ~ snususe  + sex + livingalone +
                       education + inc.90 +
                       us.ar + 
                       Boende + late 
                    + strata(alder.VIP), data = vip0)
fit.ph <- coxph(Surv(exit, event) ~ snususe + sex + livingalone +
                   education + inc.90 +
                   us.ar + Boende + 
                   late 
               + strata(alder.VIP)
                , data = vip0[vip0$alder.VIP == "40", ])
ltx2(fit.basic, label = "tab:basic2", caption = "Stratified by alder.VIP.")
```
...
fit.ph <- coxph(Surv(exit, event) ~ snususe + sex + livingalone +
                ...
                , data = vip0[vip0$alder.VIP == "30", ])
ltx2(fit.basic, label = "tab:basic230", caption = "alder.VIP = 30.")
```

## Age group 40

```{r basic240, results = 'asis'}
fit.basic <- coxreg(Surv(exit, event) ~ snususe + sex + inc.90 +
                       us.ar + livingalone +
                       Boende + late, 
                    data = vip0[vip0$alder.VIP == "40", ])
fit.ph <- coxph(Surv(exit, event) ~ snususe + sex + livingalone +
                ...
                , data = vip0[vip0$alder.VIP == "40", ])
ltx2(fit.basic, label = "tab:basic240", caption = "alder.VIP = 40.")
```
...
fit.ph <- coxph(Surv(exit, event) ~ snususe + sex + livingalone +
                ...
                , data = vip0[vip0$alder.VIP == "50", ])
ltx2(fit.basic, label = "tab:basic250", caption = "alder.VIP = 50.")
```
...
fit.ph <- coxph(Surv(exit, event) ~ snususe + sex + livingalone +
                ...
                , data = vip0[vip0$alder.VIP == "60", ])
ltx2(fit.basic, label = "tab:basic260", caption = "alder.VIP = 60.")
```
